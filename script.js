...
function renderSessionPage(){
 ...
 <section class="container session-content">
   ...
   <aside class="sidebar">
     <div class="action-card"><h3>Join the session</h3><p>When the live link is ready, add it here in the session data.</p><a class="placeholder-button" href="#" onclick="return false">Teams link coming soon</a></div>
     <div class="speaker-box"><h3>Presenter</h3><p><strong>${esc(s.speaker)}</strong></p>${s.organization?`<p>${esc(s.organization)}</p>`:""}</div>
     <div class="action-card" style="background:#eef4f0;color:var(--deep)"><h3>Recording</h3><p style="color:#607274">A recording link can be added after the conference.</p><span class="placeholder-button">Available after event</span></div>
   </aside>
 </section>`;
